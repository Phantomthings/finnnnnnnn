...
def _pdf_kpi_table(kpis, styles):
    n = len(kpis)
    cells = [[
        [Paragraph(str(v), styles["kpi_val"]), Paragraph(lbl, styles["kpi_lbl"])]
        for v, lbl in kpis
    ]]
    col_w = (A4[0] - 3 * cm) / n
...
def _build_pdf_export(
    ...
    period = f"Période : {date_debut_str} → {date_fin_str}"
    if sites_list:
        period += f"  |  Sites : {', '.join(sites_list)}"
    elems.append(Paragraph(period, styles["subtitle"]))
    elems.append(Paragraph(f"Généré le {date_generation}", styles["small"]))
    elems.append(HRFlowable(width="100%", thickness=1, color=_PDF_BORDER, spaceAfter=8))

    elems.append(Paragraph("Résumé global", styles["section"]))
    taux_color = "#16a34a" if taux_reussite >= 90 else ("#d97706" if taux_reussite >= 80 else "#dc2626")
    kpis = [
        (f'<font color="{taux_color}">{taux_reussite}%</font>', "Taux de réussite"),
        (f"{total_charges:,}", "Total charges"),
        (f'<font color="#16a34a">{ok_charges:,}</font>', "Charges OK"),
        (f'<font color="#dc2626">{nok_charges:,}</font>', "Charges NOK"),
        (f"{energy_total:.1f} kWh", "Énergie totale"),
        (str(sites_actifs), "Sites actifs"),
    ]
    ...
    if site_performance:
        ...
        col_w = A4[0] - 3 * cm
        widths = [col_w*0.30, col_w*0.10, col_w*0.10, col_w*0.10, col_w*0.18, col_w*0.22]
        rows_data = []
        ...
        elems.append(_pdf_data_table(
            ["Site", "Total", "OK", "NOK", "Taux OK (%)", "Énergie (kWh)"],
    ...
    if top_errors:
        ...
        col_w = A4[0] - 3 * cm
        widths = [col_w*0.06, col_w*0.54, col_w*0.18, col_w*0.22]
        rows_data = [
            [str(e["rank"]), str(e["type"]), str(e["count"]), f"{e['percentage']}%"]
            for e in top_errors
        ]
        elems.append(_pdf_data_table(
            ["#", "Type d'erreur", "Occurrences", "% des erreurs"],
            rows_data, styles, widths,
        ))
        elems.append(Spacer(1, 8))

    if moment_distribution:
        elems.append(Paragraph("Répartition des erreurs par moment", styles["section"]))
        col_w = A4[0] - 3 * cm
        widths = [col_w*0.50, col_w*0.25, col_w*0.25]
        rows_data = [
            [m["moment"], str(m["count"]), f"{m['percentage']}%"]
            for m in moment_distribution
        ]
        elems.append(_pdf_data_table(
            ["Moment", "Occurrences", "% des erreurs"],
            rows_data, styles, widths,
        ))
        elems.append(Spacer(1, 8))

    if points_forts or points_attention or recommandations:
        elems.append(Paragraph("Insights & Recommandations", styles["section"]))
        if points_forts:
